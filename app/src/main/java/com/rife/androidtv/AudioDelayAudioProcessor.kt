...
class AudioDelayAudioProcessor : AudioProcessor {
    ...
    private fun recalculateDelayBytes() {
        ...
        if (delayMs > 0) {
            val targetBytes = (delayMs * bytesPerMs).toInt().coerceAtLeast(1)
    ...
    override fun queueInput(inputBuffer: ByteBuffer) {
        ...
        if (delayMs > 0 && delayBytesCount > 0 && delayBuffer.isNotEmpty()) {
            ...
            while (inputBuffer.hasRemaining()) {
                ...
                if (bufferedByteCount < delayBytesCount) {
                    ...
                    bufferWriteIndex = (bufferWriteIndex + 1) % bufSize
                    bufferedByteCount++
                } else {
                    ...
                    delayBuffer[bufferWriteIndex] = bIn
                    bufferReadIndex = (bufferReadIndex + 1) % bufSize
                    bufferWriteIndex = (bufferWriteIndex + 1) % bufSize
                }
            }
            outputBuffer.flip()
